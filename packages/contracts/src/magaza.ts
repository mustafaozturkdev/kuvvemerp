...
export const MagazaGuncelleSemasi = z.object({
  kod: z.string().min(1).max(50),
  ad: z.string().min(1).max(200),
  tip: z.string().max(20),
  ilAdi: z.string().max(50).nullable(),
  ilceAdi: z.string().max(50).nullable(),
  adres: z.string().max(1000).nullable(),
  telefon: z.string().max(30).nullable(),
  cep: z.string().max(30).nullable(),
  email: z.string().email().max(200).nullable(),
  ip: z.string().max(50).nullable(),
  instagram: z.string().max(100).nullable(),
  eFaturaOnEk: z.string().max(3).nullable(),
  eArsivOnEk: z.string().max(3).nullable(),
  harita: z.string().max(5000).nullable(),
  paraBirimiKod: z.string().length(3),
  ulkeKodu: z.string().length(2),
}).partial();
